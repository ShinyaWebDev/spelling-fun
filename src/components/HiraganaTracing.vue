<template>
    <v-card class="pa-6 mb-6" elevation="3" rounded="xl" style="background: linear-gradient(135deg, #e8f5e8, #f0fff0)">
        <h3 class="text-h5 mb-4 text-center">✏️ Trace the Hiragana: {{ character }}</h3>
        <div class="text-center mb-2 text-body-2">
            Strokes: {{ userStrokes.length }} / {{ hiraganaData[character]?.strokes || 1 }}
        </div>
...
function calculateAccuracy() {
        ...
        const closestUserPoint = userPoints.value.reduce(
            (closest, userPoint) => {
                const distance = Math.sqrt(
                    Math.pow(keyPoint[0] - userPoint.x, 2) + Math.pow(keyPoint[1] - userPoint.y, 2)
                );
                return distance < closest.distance ? { point: userPoint, distance } : closest;
            },
            { distance: Infinity }
        );
    ...
    accuracy.value = matchedPoints > 0 ? Math.max(0, 100 - totalDistance / matchedPoints) : 0;
}
